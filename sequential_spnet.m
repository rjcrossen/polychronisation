...
% By default, it saves to "./Networks" folder
...
sim_length=60;          % simulation length (seconds)
ratio=4;                % ratio of excitatory:inhibitory
...
for h=1:length(N_vec)
    ...
    Ne=(N_vec(h)/(ratio+1)) * ratio; % excitatory neurons                   
    Ni= N_vec(h) - Ne;                 % inhibitory neurons                         
    ...
    % Network loop
    for sec=1:sim_length % Simulating one minute
    ...
    save(save_dir + int2str(N) + ".mat")
    disp("Sim with " + int2str(N) + " neurons complete")
